...
for(var i = 0; i < 7; addData(day[i], Math.round(Math.random() * 20)), i++);
C2("u" + myLineChart.data.datasets[0].data[6].toString());

var active = setInterval(function(){
	let price = Math.round(myLineChart.data.datasets[0].data[6] * sum);
	updateData(price > 9 ? price : 10);
	sum = setSum();
}, 5000);
...
function setSum() {
	let sum = 0;
	let count = 0;
	let effect = [1.3, 0.9, 0.9, 0.8, 1.2];
	let news = ['มีประเทศใหม่ที่ยอมรับ Bitcoin','มีสกุลเงินดิจิทัลใหม่มาเปิดตัว','มีความพยายามจากรัฐบาลในการควบคุม Bitcoin','มีข่าว Hacker พยายามทำการเจาะระบบ Bitcoin','การ์ดจอขาดตลาด'];
	newsBox.innerHTML = "";
	for (let i=0; i < effect.length; i++) {
		if (Math.random() > 0.7) {
			sum += effect[i];
			count++;
			newsBox.appendChild(document.createTextNode(news[i] + " // "));
		}
	}
...
function updateData(price) {
	myLineChart.data.labels.push(myLineChart.data.labels[0]);
